...
{
    global $conn;
    $query = $conn->query("SELECT dpc FROM appartamenti WHERE IdAppartamento=$IdAppartamento");
    if ($query->num_rows > 0) {
        $row = $query->fetch_assoc();
        if ($row['dpc'] == 0) {
            echo '<p>Non ci sono prenotazioni a carico.</p>';
        } else if ($row['dpc'] == 1) {
            $query2 = $conn->query("SELECT DataInizio, DataFine FROM prenotazioni WHERE FK_IdAppartamento =$IdAppartamento");
            if ($query2->num_rows > 0) {
                while ($row2 = $query2->fetch_assoc()) {
                    echo '<p>Affittato dal ' . date('d/m/Y', strtotime($row2['DataInizio'])) . ' al ' . date('d/m/Y', strtotime($row2['DataFine'])) . '.</p>';
                }
            }
        }
    }
